...
users_recs_redis = Redis(app, config_prefix="REDIS_USERS_RECS")
...
catalog = Catalog(app).load(
    ...
    app.config["USERS_RECS_PATH"]
)
catalog.upload_tracks(tracks_redis.connection)
catalog.upload_artists(artists_redis.connection)
catalog.upload_users_recs(users_recs_redis.connection)
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        treatment = Experiments.MY_EXP.assign(user)
        if treatment == Treatment.T1:
            recommender = UsersRecs(catalog,
                                    users_recs_redis.connection,
